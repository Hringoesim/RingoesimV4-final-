...
serve(async (req) => {
  ...
  try {
    console.log('Waitlist signup function called');
...
Questions? Contact us at info@ringoesim.com`
    ...
    if (!userEmailResponse.ok) {
      ...
      throw new Error(`Failed to send confirmation email: ${userEmailResponse.status}`);
    }
...
This is an automated notification from the Ringo waitlist system.`
    ...
    if (!adminEmailResponse.ok) {
      ...
      // Don't throw error for admin email failure - user signup should still succeed
    } else {
    ...
    return new Response(
      JSON.stringify({ 
        ...
        user_email_id: userEmailResult.id 
      }),
      { 
        status: 200, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
